...
def start_yolo_detection():
    ...
    try:
        ...
        # 启动YOLO检测
        cmd2 = "roslaunch ultralytics_ros tracker.launch debug:=true"
...
def stop_yolo_detection():
    ...
    try:
        # 查找相关进程
        cmd = "ps aux | grep -E 'ultralytics_ros|dabai_u3.launch' | grep -v grep"
        ...
        # 关闭所有相关进程
        subprocess.run("pkill -f 'roslaunch ultralytics_ros'", shell=True)
        subprocess.run("pkill -f 'roslaunch astra_camera dabai_u3.launch'", shell=True)
        
        print("YOLO目标检测已停止")
        
    except Exception as e:
        print(f"停止YOLO目标检测时出错: {e}")

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法: python3 module23_yolo.py [start|stop]")
        sys.exit(1)
